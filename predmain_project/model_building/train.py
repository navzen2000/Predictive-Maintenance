...
# 1. Identify the top-performing method name from the summary table
best_method_name = summary_df.iloc[0]['method']

# 2. Extract the actual fitted pipeline object for that method
best_model = summary_df.iloc[0]['best_model']

# 3. Get Best Params
best_params = summary_df.iloc[0]['best_params']

print(f"The best model selected is: GradientBoostingClassifier + {best_method_name}")
...
# 1. Update to your optimized F-beta threshold for Class 0
classification_threshold = best_threshold

# 2. Get probabilities for CLASS 1 (Unhealthy)
# Using X_train and X_test directly as best_model is a Pipeline
y_pred_train_proba_1 = best_model.predict_proba(X_train)[:, 1]
y_pred_test_proba_1  = best_model.predict_proba(X_test)[:, 1]

# 3. Apply threshold directly to Class 1
# If prob >= threshold, label it Unhealthy (1), else Healthy (0)
y_pred_train = np.where(y_pred_train_proba_1 >= classification_threshold, 1, 0)
y_pred_test  = np.where(y_pred_test_proba_1 >= classification_threshold, 1, 0)

# ====================== PRINT REPORTS ======================
print("\n" + "="*60)
print(f"FINAL MODEL PERFORMANCE (Threshold for Unhealthy = {classification_threshold:.4f})")
print("="*60)

print("\nTrain Metrics:")
train_report = classification_report(y_train, y_pred_train, target_names=['Healthy (0)', 'Unhealthy (1)'], output_dict=True)
print(classification_report(y_train, y_pred_train, 
                            target_names=['Healthy (0)', 'Unhealthy (1)']))

print("\nTest Metrics:")
test_report = classification_report(y_test, y_pred_test, target_names=['Healthy (0)', 'Unhealthy (1)'], output_dict=True)
print(classification_report(y_test, y_pred_test, 
                            target_names=['Healthy (0)', 'Unhealthy (1)']))

# ====================== FOCUS ON BUSINESS CRITICAL METRICS ======================
print("\n" + "-"*50)
print("BUSINESS CRITICAL METRICS (Class 1 = Unhealthy)") # Fixed header
print("-"*50)
...
for dataset, y_true, y_pred in [("Train", y_train, y_pred_train), ("Test", y_test, y_pred_test)]:
    ...
    # Synchronized variable name here:
    fbeta_1 = fbeta_score(y_true, y_pred, beta=2, pos_label=1)
   
    print(f"\n{dataset} Set:")
    print(f" Recall on Unhealthy (1)    : {recall_1:.4f} ← How many bad engines we catch")
    print(f" Precision on Unhealthy (1) : {precision_1:.4f} ← How trustworthy our 'Unhealthy' alerts are")
    print(f" F-beta=2 on Unhealthy (1)  : {fbeta_1:.4f} ← Business score")

    if dataset == "Train":
        train_recall, train_precision, train_fbeta = recall_1, precision_1, fbeta_1
    else:
        test_recall, test_precision, test_fbeta = recall_1, precision_1, fbeta_1


# Log all metrics to MLflow (including F-beta)
mlflow.log_metrics({
    "train_precision": train_precision,
    "train_recall": train_recall,
    "train_fbeta2": train_fbeta,                    
    
    "test_precision": test_precision,
    "test_recall": test_recall,
    "test_fbeta2": test_fbeta                      
})
...
# Upload the threshold file to the same repository
api.upload_file(
